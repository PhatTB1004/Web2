            ...
            if (!$canCancel) {
                ...
                $sqlUpdate = "
                    ...
                    SET status = 'cancelled',
                        cancel_reason = 'Người dùng hủy đơn',
                        cancel_by = 'user'
                    WHERE id = " . $orderId . "
                      AND user_id = " . (int) $user['id'] . "
                    LIMIT 1
                        ...
                        <table class="table table-bordered table-hover orders-table mb-0">
                           <thead>
                              <tr>
                                 ...
                                 <th style="width: 50px; text-center">Số lượng</th>
                           ...
                           <tbody>
                              <?php foreach ($orders as $order): ?>
                                 ...
                                 $expectedDate = !empty($order['expected_delivery_date']) ? date('d/m/Y', strtotime($order['expected_delivery_date'])) : '';
                                 $receivedDate = !empty($order['date_received']) ? date('d/m/Y', strtotime($order['date_received'])) : '';
                                 if ($receivedDate === '' && !empty($order['delivery_date'])) {
                                     $receivedDate = date('d/m/Y', strtotime($order['delivery_date']));
                                 }
                                 $displayReceiveDate = $receivedDate !== '' ? $receivedDate : ($expectedDate !== '' ? 'Dự kiến ' . $expectedDate : '—');
                                 ...
                                 if (in_array($statusRaw, ['pending', 'new', 'unprocessed'], true)) {
                                     ...
                                     if (($order['cancel_by'] ?? '') === 'admin') {
                                         $statusText = 'Đơn hàng đã bị admin hủy';
                                     } elseif (($order['cancel_by'] ?? '') === 'user') {
                                         $statusText = 'Bạn đã hủy đơn hàng';
                                     } else {
                                 ...
                                 <tr>
                                    ...
                                    <td>
                                       <span class="date-pill"><?php echo h($displayReceiveDate); ?></span>
                                    </td>
                                    ...
                                    <td>
                                       ...
                                       <?php if ($statusRaw === 'cancelled' && $cancelReason !== ''): ?>
                                          <div class="cancel-note">Lý do hủy: <?php echo h($cancelReason); ?></div>
                                       <?php endif; ?>
                                    </td>
