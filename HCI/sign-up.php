...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
    ...
    } elseif (!preg_match('/^0\d{9}$/', $phone)) { // Ràng buộc số điện thoại
        $error = 'Số điện thoại phải là 10 chữ số và bắt đầu bằng số 0.';
    } elseif (!preg_match('/^[A-Za-zÀ-ỹ\s]+$/', $receiver_name)) { // Ràng buộc họ tên
        $error = 'Họ tên chỉ được chứa chữ cái và khoảng trắng.';
    } elseif (!preg_match('/^[A-Za-z0-9À-ỹ\s\/]+$/', $address_detail)) { // Ràng buộc địa chỉ
        $error = 'Địa chỉ chỉ được chứa chữ cái, số và dấu gạch chéo ("/").';
    } else {
        ...
        if ($exists) {
            ...
            $status = 'active';
            $role = 'customer'; // Tài khoản đăng ký là khách hàng
            $phoneUser = $phone;
