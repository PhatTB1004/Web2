...
<div id="content-page" class="content-page"><div class="container-fluid">
   ...
   <div class="iq-card"><div class="iq-card-body table-responsive"><table class="table table-striped table-bordered"><thead><tr><th>#</th><th>Sách</th><th>Phân loại</th><th>Tồn hiện tại</th><th>Tồn tại thời điểm</th><th>Trạng thái</th></tr></thead><tbody><?php $stt=1; foreach ($rows as $row): $stock = (int) $row['stock_at_to']; ?><tr><td><?php echo $stt++; ?></td><td><?php echo h($row['bookname']); ?></td><td><?php echo h($row['category_names']); ?></td><td><?php echo (int) $row['stock_quantity']; ?></td><td><?php echo $stock; ?></td><td><?php if ($stock <= $threshold): ?><span class="badge badge-danger">Sắp hết hàng</span><?php else: ?><span class="badge badge-success">Còn hàng</span><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div></div>
