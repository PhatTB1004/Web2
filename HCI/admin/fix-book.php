...
$newImage = upload_file('image', '../images/books');
if ($newImage) {
    if ($image) {
        delete_file_if_exists($image);
    }
    ...
    $stmt = mysqli_prepare(db(), 'UPDATE books SET image = ?, bookname = ?, author_id = ?, info = ?, cost_price = ?, profit_percent = ?, sell_price = ?, stock_quantity = ?, updated_at = NOW(), status = ? WHERE id = ?');
    mysqli_stmt_bind_param($stmt, 'ssisdddisi', $image, $bookname, $authorId, $info, $costPrice, $profitPercent, $sellPrice, $stockQuantity, $status, $id);
        ...
        <div class="iq-card">
            ...
            <div class="iq-card-body">
                <form method="post" enctype="multipart/form-data"><input type="hidden" name="id"
                    ...
                    <div class="row">
                        <div class="col-md-4 form-group"><label>Mã sách</label><input class="form-control"
                                value="<?php echo h($book['book_code']); ?>" disabled></div>
                        <div class="col-md-8 form-group"><label>Tên sách</label><input name="bookname"
                                class="form-control" value="<?php echo h($book['bookname']); ?>" required></div>
                        <div class="col-md-6 form-group"><label>Tác giả</label><select name="author_id"
                                class="form-control" required><?php foreach ($authors as $a): ?><option
                                    value="<?php echo (int) $a['id']; ?>"
                                    <?php echo ((int) $book['author_id'] === (int) $a['id']) ? 'selected' : ''; ?>>
                                    <?php echo h($a['fullname']); ?></option><?php endforeach; ?></select></div>
                        <div class="col-md-6 form-group"><label>Ảnh hiện tại</label>
                            <div class="mb-2"><?php if (!empty($book['image'])): ?><img
                                    src="../images/books/<?php echo h($book['image']); ?>"
                                    style="max-width:120px;border-radius:8px;" alt=""><?php else: ?>Chưa có
                                ảnh<?php endif; ?></div><label><input type="checkbox" name="remove_image" value="1"> Bỏ
                                hình</label><input type="file" name="image" class="form-control-file mt-2"
                                accept="image/*">
                        </div>
                        <div class="col-md-12 form-group"><label>Phân loại</label><select name="category_ids[]"
                                class="form-control" multiple required><?php foreach ($categories as $cat): ?><option
                                    ...
                                    <?php echo in_array((int) $cat['id'], $selectedCategories, true) ? 'selected' : ''; ?>>
                                    <?php echo h($cat['name']); ?></option><?php endforeach; ?></select></div>
                        <div class="col-md-12 form-group"><label>Mô tả</label><textarea name="info" rows="4"
                                class="form-control"><?php echo h($book['info']); ?></textarea></div>
                        <div class="col-md-3 form-group"><label>Giá vốn</label><input type="number" step="0.01" min="0"
                                name="cost_price" class="form-control" value="<?php echo h($book['cost_price']); ?>">
                        </div>
                        ...
                        <div class="col-md-3 form-group"><label>Số lượng tồn</label><input type="number" step="1"
                                min="0" name="stock_quantity" class="form-control"
                                value="<?php echo (int) $book['stock_quantity']; ?>"></div>
                        <div class="col-md-3 form-group"><label>Hiện trạng</label><select name="status"
                                class="form-control">
                                <option value="visible" <?php echo $book['status'] === 'visible' ? 'selected' : ''; ?>>
                                    visible</option>
                                <option value="hidden" <?php echo $book['status'] === 'hidden' ? 'selected' : ''; ?>>
                                    hidden</option>
                            </select></div>
